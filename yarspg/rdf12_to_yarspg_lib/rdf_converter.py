#!/usr/bin/env python3
# Based on rdf12conv 0.3.0 with local terminology updates.
# Copyright (c) 2026 Dominik Tomaszuk
...
@dataclass(frozen=True)
class TripleTerm:
    """Quoted triple term used as an RDF 1.2 node value."""
    subject: IRI | BNode
    predicate: IRI
...
class NTriplesParser(BaseParser):
    ...
    def parse_object(self) -> Node:
        ...
        if self.scanner.startswith("<<"):
            self.scanner.error("invalid RDF 1.2 quoted triple syntax, expected '<<( ... )>>'")
        ch = self.scanner.peek()
        if ch == "<":
            return IRI(self.parse_iri(require_absolute=True))
        if self.scanner.startswith("_:"):
